...
class AmharicImeService : InputMethodService(),
    ...
    /**
     * Shows the current typing mode (Ge'ez or English) as a single entry
     * in the candidate bar whenever there's no active word being composed.
     */
    private fun showModeIndicator() {
        val label = if (isGeezMode) "ግዕዝ (Ge'ez) — tap 🌐 for English" else "English — tap 🌐 for Ge'ez"
        candidateViewManager?.updateCandidates(listOf(label), null)
    }
